...
class Elevator
{
	enum Direction{ UP, DOWN };//for selecting direction
	Direction direction;

	std::vector<int> requests = {};//ENTER THE USER REQUEST
 	int min_floor; //Undergroud floor will be shown using negative value
 	int max_floor;
 	int current_floor = 0; //ground floor
 	std::size_t passengers = 0;
 	std::size_t capacity;
//constructor which initialize the requirement
 public:
...
void Elevator::set_request() //take request as input from user
{
	...
	std::cout << "\nEnter \"GO\" if no one enters from the floor Or to exit from program if elevator is idle\n";
